...
public class E01WrapperClass1 {
	...
	public static void main(String[] args) {
		...
		/*
		 JDK1.5 이전 버전에서의 코딩 형태
		 Integer 클래스를 통해 객체화하고(박싱),
		 intValue() 메소드를 통해 기본자료형으로 변환한다.(언박싱)
		 */
		...
		/*
		 JDK1.5버전부터의 코딩형태
		 기본자료형과 객체간의 형변환이 아니라 자동으로 박싱, 언박싱 처리가 됨
		 */
		Integer numObj1 = 100; // 오토 박싱
		Integer numObj2 = 200; // 오토 박싱
		int num3 = numObj1; // 오토 언박싱
		int result3 = numObj1 - numObj2; // 오토 언박싱
		System.out.println("오토언박싱 이후 연산결과:" + result3);

		/*
		 Wrapper 클래스에서 제공되는 메소드
		  - 진법 변환, int형으로 표현가능한 범위 등의 기능을 제공한다.
		 */
		...
		/*
		 ...
		 그러므로 아래 메소드를 사용할 수 있다.
		  - toString() : 인스턴스 변수를 출력시 참조값이 아니라 실제 저장된 값을 문자열 형태로 반환.
