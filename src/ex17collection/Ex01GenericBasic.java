...
public class Ex01GenericBasic {
	...
	public static void main(String[] args) {
		...
		/*
		 상자1은 오렌지 전용박스(Orange 타입으로 선언)이므로 다른 과일(객체)를 저장할 수 없다.
		 컴파일 에러가 발생하므로 실행자체가 불가능하다.
		 -> 즉, 자료형에는 안전하지만, 구현에는 불편함이 있다
		 	다른 객체를 저장하기 위해서는 또 다른 Box클래스를 생성해야한다.
		 */
//		oBox1.store("당도가 20인 오렌지"); - 컴파일 에러 발생, ()안이 오렌지 타입이 아니기 때문
...
//		orange2.showInfo();
		...
		FruitBox fBox2 = new FruitBox("당도가 30인 오렌지"); // Object 타입이므로 넣을 순 있음
		Orange orange4 = (Orange)fBox2.pullOut(); // 런타임 에러(예외) 발생, 형변환 불가능
		orange4.showInfo();
	}
