...
class FruitSeller{
	...
	// -> 멤버상수의 경우는 선언과 동시에 초기화해야하며 상수이므로 당연히 이후에 값을 변경할 수 없다
	//	   멤버변수는 선언과 후 초기화 혹은 값의 변경이 언제든지 가능하다
...
class FruitBuyer{
	...
	/*
	 구매자가 사과를 구매하는 행위를 표현한 멤버메소드
	  1. 판매자에게 금액을 지불한다.
	  2. 판매자가 금액에 해당하는 사과의 갯수를 반환해준다.
	  3. 반환한 갯수만큼 보유한 사과의 갯수가 증가한다.
	  4. 판매자에게 지불한 금액만큼 차감된다.
	 */
