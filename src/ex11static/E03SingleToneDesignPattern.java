...
/*
 싱글톤 디자인 패턴
  - 클래스를 설계하는 디자인 패턴의 하나로써
   	하나의 인스턴스 즉, 하나의 메모리를 생성해 이를 공유해서 사용하고 할 때 사용하는 패턴
  - 작성방법
  	1. 생성자의 접근지정자를 private으로 정의한다.
  	2. 외부클래스에서 생성자를 접근할 수 없으므로 new를 통해 객체생성은 불가능해진다.
  	3. 클래스 내부에 정적메소드로 해당 클래스의 객체를 반환하도록 정의한다.(getInstance()를 사용)
  	4. 위 방법을 통해 객체를 생성하면 메로리에 딱 하나만 만들어지게 된다.
 */
class NoSingleTone {
	int instVar;
	public NoSingleTone() {
...
class SingleTone {
	int shareVar;
	/*
	 JVM(자바가상머신)에 의해 프로그램이 시작될 대 정적변수는 미리 메소드 영역에 생성되어 사용할 준비를 마치게 된다.
	 */
...
public class E03SingleToneDesignPattern {
	...
	public static void main(String[] args) {
		...
		// 일반적인 방식의 객체 생성이므로 객체를 생성할 때마다 새로운 참조값(주소값)을 할당받는 인스턴스가 된다.
		...
		//SingleTone st1 = new SingleTon(); //에러발생
