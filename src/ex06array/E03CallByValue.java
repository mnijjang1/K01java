...
/*
 CallByValue(값에 의한 호출)
  - 메소드 호출과 관련된 개념으로 매개변수를 통해 실제 값을 메모리의 '복사'를 통하여 값이 전달한다.
  - 그래서 다른 메소드(지역)에서 값의 변경이 있더라도 호출한 메소드의 값에는 전혀 영향을 미치지 않는다.
  - 메모리적 관점에서 보면 스택영역만 사용하는 케이스로 main과 호출된 메소드는 서로 다른 지역이기 때문에
...
public class E03CallByValue {
	...
	public static void callByValue(int fNum, int sNum) {
		/*
		 스왑(Swap)
		  - 두 개의 변수가 할당받은 값을 서로 교환하는 것을 말한다.
		  - 교환시에는 임시변수 하나가 추가로 필요하다.
		 */
		int temp;
		temp = fNum;
		fNum = sNum;
