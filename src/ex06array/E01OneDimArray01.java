...
public class E01OneDimArray01 {
	...
	public static void main(String[] args) {
		...
		System.out.println("배열명(numArr)=" + numArr); // 해당 배열이 할당된 메모리의 참조값 [I@15db9742가 출력
		System.out.println("numArr 2번방의 값:" + numArr[2]); // 해당 배열에 들어있는 30 출력
		System.out.println("===========================");
		...
		for(int i=0; i<40; i++) { // 같은 결과가 되더라도 장기적으로 봤을 때,
								  // 40보다는 23라인처럼 arrNumber.length을 사용하는게 수정이 편하다
		...
		/*
		 객체배열
		  - 기본자료형을 저장하는 배열이 아니라, 객체의 참조값을 저장할 용도로 생성하는 배열
		  - 사용방법은 동일하다.
		 */
		...
		/*
		 배열의 선언과 동시에 초기화를 하는 경우
		  - 자바 컴파일러는 초기값의 갯수를 통해 크기를 결정할 수 있으므로
		  	이런 경우 크기는 반드시 생략하도록 약속되어 있다.
		 */
		System.out.println("방법2 : 선언과 동시에 초기화1");
//		int[] initArr2 = new int[3] {1, 20, 300}; 에러발생 -> 1,20,300으로 크기를 알수 있기 때문에 3을 생략!
