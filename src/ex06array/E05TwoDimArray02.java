...
public class E05TwoDimArray02 {
	...
	/*
	 시나리오] 다음 절차에 따라 프로그램을 작성하시오.
	  ...
	  4. 증가되기 전, 후 배열을 출력한다.
	  5. 다음 메소드명으로 구현한. twoDimPlus()
	 */
		
		//세로2, 가로4 인 배열 선언
		int[][] twoArray = new int[2][4];
		for (int i=0; i<twoArray.length; i++) { //세로(행)크기만큼 반복
			for (int j=0; j<twoArray[i].length; j++) {	//가로(열)크기만큼 반복
				twoArray[i][j] = (int)(Math.random() * 100); // 난수를 이용해서 각 요소를 초기화
			}
		}
	...
	static void twoDimPlus(int[][] arr2, int plusNum) {
		for (int i=0; i<arr2.length; i++) {
			for (int j=0; j<arr2[i].length; j++) {
				arr2[i][j] += plusNum; // 매개변수로 전달받음 plusNum만큼각 요소를 증가시킴
			}
		}
	}
