...
public class E07ForEachBasic {
	...
	public static void main(String[] args) {
		...
		System.out.println("기본 for문을 이용한 합게 : "+sum);
		
		// 배열 arr의 요소를 하나씩 참조하여 출력하고, 더 이 상 참조할 요소가 없을 때 for문이 자동으로 종료.
		sum = 0;
		for (int e : arr) {
			System.out.print(e+" ");
			sum += e;
		...
		for (int c: arr) {
			System.out.print(c+" "); // 1이 증가된 부분이 기존 배열에 적용되지 않으므로 기존의 값이 그대로 출력.
		}
		System.out.println();
		
		/*
		 2차원 배열은 1차원 배열 여러개를 쌓아놓은 형태이므로 출력시에는 각각의 행(1차원 배열)을 가져와서
		 2중 foreach문을 통해 각 요소를 하나씩 참조하여 출력한다.
		 */
