	...
	public static void main(String[] args) {
		try {
			...
			if (con!=null) {
				...
				/*
				 3. 쿼리문 작성
				 	- 쿼리문을 작성할 때 주의할 점은 줄바꿈을 할 때 ""속의 앞뒤로 스페이스를 삽입하는 것이 좋다.
				 	  그렇지 않으면 오라클 쿼리의 문장이 이어지게 되어 syntaxError가 발생하게 된다.
				 	- 쿼리문 작성시 SELECT, FROM, WHERE 등은 대문자로 작성하면 구분하기 편하다.
				 */
				...
				/*
				 4. 쿼리문으 Oracle로 전송하기 위한 Statement 계열의 인터페이스를 사용한다.
				...
				/*
				 5. Statement 객체를 사용하여 Oracle로 작성된 쿼리문을 전송하고 실행한다.
				 	실행후 결과를 반환하면 ResultSet 객체를 통해 받는다.
				 */
				...
				while(rs.next()) {
					/*
					 ...
					 인자값으로는 컬럼의 순서에 따른 인덱스를 쓸 수도 있고, 컬럼명을 그대로 쓸 수도 있다.
					 */
					String emp_id =rs.getString(1);// 1번 컬럼
				...
				/*
				 6. 자원반납
				 	- 모든 작업을 마친후에는 메모리 절약을 위해 연결했던 자원을 반납한다.
				 */
				rs.close();
				stmt.close();
				con.close();
