	...
	public void execute() {
		try {
			...
			String query = "SELECT id, pass, name, regidate,"
					+ " to_char(regidate, 'yyyy.mm.dd hh24:mi') d1 from member";
			...
			while(rs.next()) {
				...
				/*
				 오라클의 date 타입을 getDate()로 추출하면 0000-00-00 형태로 출력된다.
				 이 경우 date형 자료가 되기 때문에 java.sql.Date클래스의 참조변수로 저장해야한다.
				 */
				...
				System.out.printf("%-5s %-4s %-13s %-10s %s %s %s\n",
