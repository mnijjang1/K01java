...
/*
 Thread(쓰레드)
  ...
  - 만약 해당 클래스가 다른 클래스르 상속받아야 한다면 Runnable 인터페이스를 구현하여 사용한다.
 */
class ShowThread extends Thread {
	...
	/*
	 ...
	 Thread 클래스의 run()메소드를오버라이딩 한 것으로 해당 메소드는 직접 호출하면 안되고,
	...
	public void run() {
		for (int i=1; i <=100; i++) {
			...
			try {
				/*
				 Thread 클래스와 static 메소드로 실행의 흐름을 주어진 시간만큼
				 Block 상태로 전환시켜준다.
				 1000분의 1초 단위(1ms)로 기술하게 되므로 아래는 0.01초만큼 Block시키게 된다.
				 */
				sleep(10);
			}
...
public class Ex01ThreadStart {
	...
	public static void main(String[] args) {
		...
		/*
		 run() 메소드는 호출은 가능하나 쓰레드가 새로 생성되지 않는다.
