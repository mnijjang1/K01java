...
/*
 람다식(lambda Expression)
  - 객체지향에서는 메소드를 사용하기 위해서는 반드시 클래스르 정의한 후 메소드를 선언해야 한다.
  	이러한 불편함 때문에 최근 함수형 프로그래밍(Functional Programming)의 장점이 대두되었고,
  	이런 단점을 보완하기 위해 JDK 1.8부터지원하게 되었다.
 */
interface IKosmo3 {
...
public class Ex03Lambda {
	...
	public static void main(String[] args) {
		...
		/*
		 위의 람다식을 아래와 같이 한번 더 줄이는 게 가능하다.
		  - 매개변수의 타입을 예상할 수 있다면 생략 가능.
		  - 매개변수가 하나라면 소괄호도 생락가능
		  - 실행할 문장이 하나이면 중괄호도 생략 가능(단, return문은 중괄호가 반드시 필요) 
		 */
